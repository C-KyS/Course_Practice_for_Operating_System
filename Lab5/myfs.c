...
int start_sys(void) {
    ...
    // 批量初始化openfile_list[1~MAX_OPENFILE-1], 后面的FCB, 初始化置空
    ...
    // 初始化各个全局变量
    strcpy(current_dir, openfile_list[curdir].dir); // current_dir 初始化为根目录
    start = ((block0 *) fs_head)->start_block; // start 初始化为第一个数据块的地址
    free(empty);
    return 0;
}
